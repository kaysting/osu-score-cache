...
/**
 * Retrieves a valid OAuth token. Handles expiration and race conditions
 * where multiple requests try to refresh the token simultaneously.
...
async function getToken() {
    ...
    tokenPromise = (async () => {
        try {
            ...
            throw new Error(`Error getting token from osu! API: ${error}`);
        } finally {
            tokenPromise = null;
        }
    })();
...
const makeGetRequest = async (endpoint, params = {}) => {
    ...
    try {
        ...
        throw new Error(`Error making GET request to osu! API ${endpoint}: ${error}\nResponse data: ${JSON.stringify(error.response?.data, null, 2)}`);
    }
};
